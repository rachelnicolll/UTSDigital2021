 ...
 546  01b3 81            	ret
 600                     ; 210 void mcu_msWait(unsigned long msWait)
 600                     ; 211 {
 601                     	switch	.text
 602  01b4               _mcu_msWait:
 604  01b4 5208          	subw	sp,#8
 605       00000008      OFST:	set	8
 608                     ; 215     delay_ms(msWait);
 611  01b6 1e0d          	ldw	x,(OFST+5,sp)
 612  01b8 1f03          	ldw	(OFST-5,sp),x
 613  01ba 1e0b          	ldw	x,(OFST+3,sp)
 614  01bc 1f01          	ldw	(OFST-7,sp),x
 616                     ; 13     for (i = 0; i < ((F_CPU / 18 / 1000UL) * ms); i++) {
 618  01be ae0000        	ldw	x,#0
 619  01c1 1f07          	ldw	(OFST-1,sp),x
 620  01c3 ae0000        	ldw	x,#0
 621  01c6 1f05          	ldw	(OFST-3,sp),x
 624  01c8 200a          	jra	L362
 625  01ca               L752:
 626                     ; 14         _asm("nop");
 629  01ca 9d            nop
 631                     ; 13     for (i = 0; i < ((F_CPU / 18 / 1000UL) * ms); i++) {
 633  01cb 96            	ldw	x,sp
 634  01cc 1c0005        	addw	x,#OFST-3
 635  01cf a601          	ld	a,#1
 636  01d1 cd0000        	call	c_lgadc
 639  01d4               L362:
 642  01d4 96            	ldw	x,sp
 643  01d5 1c0001        	addw	x,#OFST-7
 644  01d8 cd0000        	call	c_ltor
 646  01db a66f          	ld	a,#111
 647  01dd cd0000        	call	c_smul
 649  01e0 96            	ldw	x,sp
 650  01e1 1c0005        	addw	x,#OFST-3
 651  01e4 cd0000        	call	c_lcmp
 653  01e7 22e1          	jrugt	L752
 654                     ; 216 }
 657  01e9 5b08          	addw	sp,#8
 658  01eb 81            	ret
 671                     	xdef	_mcu_msWait
 672                     	xdef	_mcu_i2cTransfer
 673                     	xdef	_mcu_i2cInit
 674                     	xref	_I2C_GetFlagStatus
 675                     	xref	_I2C_CheckEvent
 676                     	xref	_I2C_SendData
 677                     	xref	_I2C_Send7bitAddress
 678                     	xref	_I2C_GenerateSTOP
 679                     	xref	_I2C_GenerateSTART
 680                     	xref	_I2C_Cmd
 681                     	xref	_I2C_Init
 682                     	xref	_I2C_DeInit
 683                     	xref	_GPIO_Init
 684                     	xref	_CLK_PeripheralClockConfig
 703                     	xref	c_lcmp
 704                     	xref	c_smul
 705                     	xref	c_ltor
 706                     	xref	c_lgadc

  ...
  61                     ; 22 	USART1->CR2 |= USART_CR2_TEN | USART_CR2_REN;
  63  000c c65235        	ld	a,21045
  64  000f aa0c          	or	a,#12
  65  0011 c75235        	ld	21045,a
  66                     ; 23 	USART1->CR3 &= ~(USART_CR3_STOP);					 // 1 stop bit (0b0011 0000)
  68  0014 c65236        	ld	a,21046
  69  0017 a4cf          	and	a,#207
  70  0019 c75236        	ld	21046,a
  71                     ; 24 	USART1->BRR2 = 0x03;								 //setting
  73  001c 35035233      	mov	21043,#3
  74                     ; 25 	USART1->BRR1 = 0x68;								 // 9600 baud
  76  0020 35685232      	mov	21042,#104
  77                     ; 27 	GPIO_Init(GPIOC, GPIO_Pin_5, GPIO_Mode_In_FL_No_IT); 
  79  0024 4b00          	push	#0
  ...
  85                     ; 28 }
  88  002f 81            	ret
 159                     ; 31 void reverse(char* str, int len)
 159                     ; 32 {
 160                     	switch	.text
 161  0030               _reverse:
 163  0030 89            	pushw	x
 ...
 375  00d0 81            	ret
 474                     ; 66 void ftoa(float n, char* res, int afterpoint)
 474                     ; 67 {
 475                     	switch	.text
 476  00d1               _ftoa:
 478  00d1 520e          	subw	sp,#14
 479       0000000e      OFST:	set	14
 482                     ; 69     int ipart = (int)n;
 ...
 492                     ; 72     float fpart = n - (float)ipart;
 ...
 515                     ; 75     int i = intToStr(ipart, res, 0);
 517  0100 5f            	clrw	x
 518  0101 89            	pushw	x
 ...
 581                     ; 93         intToStr((int)fpart, res + i + 1, afterpoint);
 ...
 645  0164               L342:
 646                     ; 99 	while (!(USART1->SR & USART_SR_TXE))
 ...
 703  0176               L762:
 704                     ; 108 	while (!(USART1->SR & USART_SR_RXNE))
 ...
 720  0186 81            	ret
 754                     ; 114 void UART_TX(unsigned char val)
 754                     ; 115 {
 755                     	switch	.text
 756  0187               _UART_TX:
 758  0187 88            	push	a
 ...
 999                     ; 139 void UART_2PC(uint16_t nbReadings, float tempResults[],float humResults[])
 999                     ; 140 {
1000                     	switch	.text
1001  01b3               _UART_2PC:
...
1008                     ; 145 	val = GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_5);
...
1025                     ; 148 		printf("Welcome to the Greenhouse Datalogger\n");
1027  01cc ae001f        	ldw	x,#L534
1028  01cf cd0000        	call	_printf
1030                     ; 149 		printf("Transmitting Data...\n");
...
1040  01df               L334:
1041                     ; 153 	for (i = 0; i < nbReadings; i++)
1043  01df 5f            	clrw	x
1044  01e0 1f0a          	ldw	(OFST-1,sp),x
1047  01e2 2039          	jra	L544
1048  01e4               L144:
1049                     ; 155 		t = tempResults[i];
...
1088                     ; 153 	for (i = 0; i < nbReadings; i++)
